  ...
  describe "when called" do
    ...
    describe 'extracting credentials' do
      ...
      it 'errors if the token on stdin is not a valid OAuth token' do
        ...
        action = described_class.new({ config: '/some/nonexistent/path',
                                       'token-path': '-' }, [])
        expect{ action.call }.to raise_error(R10K::Error, /Supplied token is not a valid OAuth token/)
      end
      ...
      it 'passes sshkey path to deployment' do
        ...
        expect(R10K::Deployment).to receive(:new).with({}, { sshkey_file: sshkey_file.path }).and_return(deployment)
        action.call
      end

      it 'writes sshkey from stdin to tempfile, passes tempfile to deployment' do
        sshkey_file = Tempfile.new('sshkey')
        expect(Tempfile).to receive(:new).and_return(sshkey_file)
        allow($stdin).to receive(:read).and_return("my_key")
        action = described_class.new({ config: '/some/nonexistent/path',
                                       'sshkey-path': '-' }, [])
        expect(R10K::Deployment).to receive(:new).with({}, { sshkey_file: sshkey_file.path }).and_return(deployment)
        action.call
